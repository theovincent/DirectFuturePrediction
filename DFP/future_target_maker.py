...
# IDEAS:
# using pandas to handle scalar data
...
class FutureTargetMaker:
    def __init__(self, args):
        ...
        if self.rwrd_schedule_type == "exp":
            ...
            self.rwrd_schedules = np.power(
                np.reshape(gammas, (-1, 1)), np.reshape(np.arange(self.future_steps[-1]), (1, -1))
            )
        else:
        ...
        for ns, sample in enumerate(indices):
            ...
            if isinstance(meas_mean, np.ndarray) and isinstance(meas_std, np.ndarray):
                targets[ns, : len(self.meas_to_predict), :] = (
                    (meas[curr_future_steps][:, self.meas_to_predict] - meas[None, sample][:, self.meas_to_predict])
                    / meas_std[:, self.meas_to_predict]
                ).transpose()
            else:
                targets[ns, : len(self.meas_to_predict), :] = (
                    meas[curr_future_steps][:, self.meas_to_predict] - meas[None, sample][:, self.meas_to_predict]
            ...
            if self.invalid_targets_replacement == "nan":
                targets[
                    ns, : len(self.meas_to_predict), invalid_samples
                ] = np.nan  # make invalid the entries which fall into the following episodes
            elif self.invalid_targets_replacement == "last_valid":
                targets[ns, : len(self.meas_to_predict), invalid_samples] = targets[
                    ns, : len(self.meas_to_predict), np.argmax(invalid_samples) - 1
                ]  # make invalid the entries which fall into the following episodes
            else:
